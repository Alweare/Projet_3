...
if (isset($_POST['nouveau_commentaire'], $id_acteur) && !empty($_POST['nouveau_commentaire'])) {
    ...
    $post_utilisateur = $bdd->prepare('SELECT id_user FROM post WHERE id_acteur = ? AND id_user = ?');
    $post_utilisateur->execute(array($id_acteur, $iduser));
    $post_utilisateur = $post_utilisateur->fetch();

    // S'il ne trouve pas l'id de l'utilisateur en cours dans la table post il permet d'insérer le commentaire en base.
    if (empty($post_utilisateur['id_user'])) {
